...
BAILEYS_API = 'http://localhost:7000'
BAILEYS_SESSION_ID = '917406213666'  # default; configurable
...
def baileys_send(receiver, text):
    """Send via Baileys. Returns (ok, msg)"""
    try:
        url = f'{BAILEYS_API}/chats/send?id={BAILEYS_SESSION_ID}'
        body = json.dumps({'receiver': receiver, 'message': {'text': text}}).encode()
...
def followup():
    ...
    html = ('<!doctype html><html><head><meta charset=utf-8>'
            ...
            'async function sendKartaWA(slug,btn){btn.disabled=true;try{const r=await fetch("/followup/api/wa-link?karta_slug="+slug);const j=await r.json();if(j.ok){window.open(j.wa_url,"_blank");showLog("WhatsApp opened — tap send in the chat");}else showLog("No phone for this karta",true);}catch(e){showLog("Error: "+e,true);}finally{btn.disabled=false;}}'
